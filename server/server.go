...
func (lobby *Lobby) promoteUser(user Client) {
	for i := 0; i < len(lobby.users); i++ {
		if lobby.users[i].username == user.username {
			debugPrintln(2, "Promoting: "+lobby.users[i].username+" to moderator")
...
func (lobby *Lobby) addSongToQueue(song Song) {
	debugPrintln(2, "adding "+song.title+" to song queue")
...
func (lobby *Lobby) msgSend(msg string) {
	...
	for _, client := range clients {
		debugPrintln(2, "Sending "+msg+" to client "+client.username)
...
func (lobby *Lobby) fileSend(file *os.File) {
	...
	file.Read(fileData)
	for _, client := range clients {
		debugPrintln(2, "Sending file to client "+client.username)
...
func createClient(config webrtc.RTCConfiguration, userName string, conn net.Conn, mod bool) Client {
	if mod {
		debugPrintln(3, "Creating client "+userName+" with mod status true")
	} else {
		debugPrintln(3, "Creating client "+userName+" with mod status false")
	}
	//Handle webRTC stuff
	...
	dataChannel.OnOpen(func() { debugPrintln(2, "Data Channel opened to "+userName) })
	pconn.OnICEConnectionStateChange(func(connState ice.ConnectionState) {
		debugPrintln(2, userName+" "+connState.String())
	})

	debugPrintln(3, "Exchanging offers")
...
func main() {
	...
	for {
	start:
		debugPrintln(1, "Accepting control connection...")
		cconn, err := ln.Accept()
		if err != nil {
